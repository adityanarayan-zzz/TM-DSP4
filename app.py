...
def build_cluster_labels(profile: pd.DataFrame) -> dict:
    ...
    for idx in profile.index:
        if cash_rank[idx] == 1 and purchase_rank[idx] >= len(profile) - 0:
            ...
            desc = (
                "Saldo & penarikan tunai (cash advance) tinggi, jarang berbelanja, "
            ...
            desc = (
                "Frekuensi & nominal pembelian tinggi (tunai maupun cicilan), limit kredit "
                "dan pembayaran besar. Nasabah bernilai tinggi (high value)."
            )
        elif payment_disc_rank[idx] == 1:
            name = "Nasabah Konservatif & Disiplin"
            desc = (
                "Saldo & penggunaan cash advance rendah, transaksi moderat, namun memiliki "
...
def preprocess(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    df["AVG_PURCHASE_TRX"] = (
        df["PURCHASES"] /
        df["PURCHASES_TRX"].replace(0, np.nan)
    )
...
with col2:
    ...
    cash_advance_trx = st.number_input(
        "Cash Advance Trx (jumlah transaksi)",
        min_value=0,
        value=0,
        step=1
    )
    ...
    purchases_trx = st.number_input(
        "Purchases Trx (jumlah transaksi)",
        min_value=0,
        value=10,
        step=1
    )
...
if st.button("🔍 Prediksi Segmen", type="primary"):
    ...
    try:
        ...
        st.success(
            f"Nasabah termasuk **Segmen {cluster_id}: {info['name']}**"
        )
